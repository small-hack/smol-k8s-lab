...
def update_user_kubeconfig():
    ...
    if path.exists(KUBECONFIG):
        ...
        with open(KUBECONFIG, 'r') as user_kubeconfig:
            existing_config = load(user_kubeconfig)

        if existing_config:
            # append new cluster, user and context
            existing_config['clusters'].extend(k3s_kubeconfig['clusters'])
            existing_config['users'].extend(k3s_kubeconfig['users'])
